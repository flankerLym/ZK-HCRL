...
$trainOut = Join-Path $OutputRoot "train_rl_all_matched"
$testOut  = Join-Path $OutputRoot "test_rl_all_matched"
if ($WorkloadMode -eq "full_trace") {
    $trainOut = Join-Path $OutputRoot "train_rl_all_fulltrace"
    $testOut  = Join-Path $OutputRoot "test_rl_all_fulltrace"
}
...
Write-Host "===== Real-trace matched RL baselines + HCRL train: first $TrainDays days ====="
Write-Host "Mode=$WorkloadMode, RequestNum=$RequestNum, TraceRiskStrength=$TraceRiskStrength"
python $runner `
  ...
  --Use_GNN_Encoder `
  --Output_Dir $trainOut `
  --Run_Tag "realtrace_${WorkloadMode}_20d_train_rlall"
...
Write-Host "===== Real-trace matched RL baselines + HCRL test: last 10 days, greedy primary/backup + guarded HCRL mode eval ====="
python $runner `
  --Real_Trace_Path $TraceCsv `
  ...
  --Output_Dir $testOut `
  --Run_Tag "realtrace_${WorkloadMode}_10d_test_rlall"
