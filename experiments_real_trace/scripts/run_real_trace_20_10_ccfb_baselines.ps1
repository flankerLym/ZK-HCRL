param(
    ...
    [int]$SafePPOTopK = 10,
    [double]$SafePPORiskWeight = 0.70,
    [double]$SafePPORepWeight = 0.35,
    [double]$SafePPOOntimeWeight = 0.25,
    [double]$SafePPOCostWeight = 0.08,
    [double]$SafePPOMaxPolicyDrop = 0.45,
    [string]$OutputRoot = "experiments_real_trace\output"
)

$ErrorActionPreference = "Stop"
$runner = "experiments_real_trace\real_trace_main.py"
$trainOut = Join-Path $OutputRoot "train_ccfb_baselines"
$testOut  = Join-Path $OutputRoot "test_ccfb_baselines"
if ($WorkloadMode -eq "full_trace") {
    $trainOut = Join-Path $OutputRoot "train_ccfb_baselines_fulltrace"
    $testOut  = Join-Path $OutputRoot "test_ccfb_baselines_fulltrace"
}
New-Item -ItemType Directory -Force -Path $trainOut | Out-Null
New-Item -ItemType Directory -Force -Path $testOut | Out-Null

# CCF-B stronger comparison set:
# 1) strong transparent rules: Reputation/Cost/Risk-aware Greedy
# 2) stronger RL: PPO and Safe-PPO
# 3) existing learned baselines + HCRL
$methods = @(
  "Reputation-Greedy", "Cost-Aware-Greedy", "Risk-Aware-Greedy",
  "DQN", "PPO", "Safe-PPO", "RA-DDQN", "PB-SafeDQN", "COBRA-Oracle", "HCRL-Oracle"
)

Write-Host "===== RealTrace CCF-B baseline training ====="
Write-Host "Mode=$WorkloadMode, RequestNum=$RequestNum, OraclesPerType=$OraclesPerType, MaliciousRatio=$MaliciousRatio"
python $runner `
  ...
  --Safe_PPO_TopK $SafePPOTopK `
  --Safe_PPO_Risk_Weight $SafePPORiskWeight `
  --Safe_PPO_Rep_Weight $SafePPORepWeight `
  --Safe_PPO_Ontime_Weight $SafePPOOntimeWeight `
  --Safe_PPO_Cost_Weight $SafePPOCostWeight `
  --Safe_PPO_Max_Policy_Drop $SafePPOMaxPolicyDrop `
  --Methods $methods `
  --Epoch $Epoch `
  --Seed $Seed `
...
Write-Host "===== RealTrace CCF-B baseline test ====="
python $runner `
  --Real_Trace_Path $TraceCsv `
  ...
  --Eval_Only `
  --Greedy_Eval `
  --Load_Weights "DQN=$($trainRun.FullName)" "PPO=$($trainRun.FullName)" "Safe-PPO=$($trainRun.FullName)" "RA-DDQN=$($trainRun.FullName)" "PB-SafeDQN=$($trainRun.FullName)" "COBRA-Oracle=$($trainRun.FullName)" "HCRL-Oracle=$($trainRun.FullName)" `
  --Output_Dir $testOut `
  --Run_Tag "ccfb_test"

if ($LASTEXITCODE -ne 0) { throw "Test failed with exit code $LASTEXITCODE" }
Write-Host "Done. Outputs are under: $trainOut and $testOut"
