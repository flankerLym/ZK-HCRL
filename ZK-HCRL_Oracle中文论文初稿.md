...
为验证 ZK-VOS 对调度合规性的判别能力，本文首先构造合法调度与多类违规调度样本，包括冷却状态违规、成员索引非法、低信誉节点、成本超限、延迟超限、风险超限和服务类型不匹配。每类场景包含 1000 个样本。实验结果表明，在所构造的合规/违规测试集中，ZK-VOS 能够正确接受全部合法调度，并拒绝全部违规调度，所有测试类型的判别准确率均达到 100%。在 witness 检查开销方面，Figure 4(a) 展示了不同合规场景下的时间分布。多数违规样例的检查时间集中在 151--157 ms 区间，而合法调度和成员合法性检查的平均时间约为 210 ms，说明该验证过程能够在较低开销下覆盖多类关键调度约束。
...
Table 3 汇总了不同方法在多随机种子下的平均表现。需要注意的是，Table 2 和 Table 3 对应不同实验设置：前者面向真实采集的 DeFi 价格请求轨迹，后者用于检验合成请求流下的训练稳定性。表中的 $n$ 表示对应方法成功完成并纳入统计的有效随机种子数量；部分基线由于训练稳定性或运行成本限制，报告两个有效随机种子的均值与标准差。ZK-HCRL Oracle 在成功率、准时成功率和恶意节点规避方面取得较优结果，其成功率达到 77.19%，准时成功率达到 99.13%，恶意节点选择率仅为 0.84%。与 PPO 相比，ZK-HCRL Oracle 的成功率更高，且恶意节点选择率更低；与 DQN、RA-DQN 等单层强化学习方法相比，ZK-HCRL Oracle 在保持较高可信节点覆盖率的同时进一步降低了恶意节点分配风险。
...
为分析各模块对整体性能的贡献，本文进一步进行消融实验。实验比较完整模型 ZK-HCRL-Full 与去除 GNN 编码器、审计反馈、约束引导、随机备份、固定单节点执行和固定并行执行等变体。完整模型并非在所有单项指标上均最优，而是在成功率、恶意节点规避、可信节点覆盖、恢复能力和单位成功成本之间取得更优综合权衡。Table 4 中 Reward 保留两位小数；当不同变体的 Reward 显示接近或相同时，本文结合成功率、成本、恶意节点选择率和可信覆盖率等多指标进行综合分析。完整结果如 Table 4 所示。
