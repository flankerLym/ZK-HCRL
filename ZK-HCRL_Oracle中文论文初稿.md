...
与传统单动作预言机选择不同，ZK-HCRL Oracle 的调度动作具有层次结构。对于时刻 $t$ 的请求，链下调度动作可表示为

$$
a_t^{sch}=(m_t,p_t,b_t),
$$

其中 $m_t$ 表示执行模式，$p_t$ 表示主预言机，$b_t$ 表示备份预言机。执行模式用于决定当前请求采用单节点执行、串行备份恢复或并行冗余执行；主预言机负责主要服务响应，备份预言机用于失败恢复、快速响应或安全冗余。该层次化动作设计能够降低复合动作空间的搜索难度，并使调度策略根据请求风险、截止时间和候选备份质量自适应调整服务编排方式。完整的可验证调度动作将在第 4 章进一步扩展为 $a_t^{ZK\text{-}HCRL}=(m_t,p_t,b_t,\pi_t^{zk})$，其中 $\pi_t^{zk}$ 表示与调度结果绑定的零知识合规证明。
