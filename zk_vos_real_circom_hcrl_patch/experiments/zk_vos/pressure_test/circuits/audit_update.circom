...
template AuditUpdate() {
    ...
    // Audit monotonicity proxy: pass should not reduce truth; fail should not increase truth.
    ...
    // If auditPass=1 use passMono; if auditFail=1 use failMono; if neither, accept.
    ...
    allOk.a <== c.out;
    allOk.b <== auditCondition;
    zkIsValid <== allOk.out;
    zkIsValid === 1;
}

component main { public [requestId, selectedOracleId] } = AuditUpdate();
